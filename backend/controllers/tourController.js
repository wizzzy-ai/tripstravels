...
const createTour = async (req, res) => {
  try {
    const { title, city, desc, maxGroupSize, photo, address, price, distance, featured, location } =
      req.body;
...
const updateTour = async (req, res) => {
  try {
    const tourId = req.params.id;
    const { title, city, photo, desc, maxGroupSize, address, price, distance, featured, location } =
      req.body;

    const newTour = await Tour.findByIdAndUpdate(
      tourId,
      { title, city, photo, desc, maxGroupSize, address, distance, price, featured, location },
      { new: true }
    );
